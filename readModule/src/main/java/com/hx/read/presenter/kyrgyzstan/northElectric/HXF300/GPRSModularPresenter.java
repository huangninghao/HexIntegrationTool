...
import com.hx.read.contact.kyrgyzstan.northElectric.HXF300.GPRSModularContact;
...
public class GPRSModularPresenter extends RxBasePresenterImpl<GPRSModularContact.View> implements GPRSModularContact.Presenter {

    private HexClientAPI clientAPI;

    public GPRSModularPresenter(GPRSModularContact.View view) {
        super(view);
        clientAPI = PortConfig.getInstance().getClientAPI();
    }

    @Override
    public void readGPRSModular() {
        if (getView() != null) {
            getView().showLoading();
        }
